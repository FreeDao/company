...
public class ShopAction extends BaseAction{
	...
	public String selCommodityType(){
		try {
			...
			if(null == commodity  || CommonUtil.validParams(commodity.getId(),commodity.getType())){
				map.put("msg", "缺少必传参数");
				map.put("code", 0);
				return SUCCESS;
			}
	...
	public String addShop(){
		try {
			...
			if(null == shop  || CommonUtil.validParams(shop.getUserId(),shop.getMenuId(),shop.getCommodityId(),shop.getSizeId(),shop.getColorId(),shop.getNum())){
				map.put("msg", "缺少必传参数");
				map.put("code", 0);
				return SUCCESS;
			}
	...
	public String delShop(){
		try {
			...
			if(null == shop  || CommonUtil.validParams(shop.getId(),shop.getUserId())){
				map.put("msg", "缺少必传参数");
				map.put("code", 0);
				return SUCCESS;
			}
	...
	public String selOrderList(){
		try {
			...
			if(null == order  || CommonUtil.validParams(order.getUserId())){
				map.put("msg", "缺少必传参数");
				map.put("code", 0);
				return SUCCESS;
			}
			
			shopService.selUserOrder(order,map);

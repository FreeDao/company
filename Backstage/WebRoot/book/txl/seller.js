...
//表格开始
	 datagrid=$('#datagrid').datagrid({
        ...
        iconCls: 'icon-save',
        methord: 'post',
        url: '../Backstage/news/selUserSeller.action',
        pageSize: 20,
        pageList:[5,10,15,20],
        frozenColumns: [[
        ...
        columns: [[
					{ field: 'email', title: '账号/手机号', width: 150,align:'center' },
					{ field: 'nickName', title: '昵称', width: 150,align:'center'},
					{ field: 'name', title: '真实姓名', width: 150,align:'center'},
					{ field: 'addtime', title: '注册时间', width: 150,align:'center'},
					{ field: 'age', title: '积分', width: 150,align:'center'}
				]],
        fit:true,
        pagination: true,
        rownumbers: true,
...
//批量删除开始
	  if (num >0) { 
		  $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
	            if (data) {
	                $.ajax({
	                    url: '../Backstage/news/delUser.action',
	                    type: 'post',
	                    data: {items: ids.join(',')},	  
	                    success: function (data) {
	                    datagrid.datagrid('reload');

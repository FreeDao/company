...
//表格开始
	 datagrid=$('#datagrid').datagrid({
        ...
        iconCls: 'icon-save',
        methord: 'post',
        url: '../Backstage/news/selShoptypetwo.action',
        pageSize: 20,
        pageList:[5,10,15,20],
        frozenColumns: [[
        ...
        columns: [[
					{ field: 'name', title: '类型', width: 150,align:'center' },
...
function newTxl(){
 ...
 function saveTxl(){
	  $('#addForm').form('submit', {
	        url:'../Backstage/news/addShoptypetwo.action',
	        onSubmit: function () {
...
//批量删除开始
	  if (num >0) { 
		  $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
	            if (data) {
	                $.ajax({
	                    url: '../Backstage/news/delShoptypetwo.action',
	                    type: 'post',
	                    data: {items: ids.join(',')},	  
	                    success: function (data) {
	                    datagrid.datagrid('reload');
...
{ field: 'addtime', title: '添加时间', width: 150,align:'center'},
    ...
    //search结束
   var url = "../Backstage/news/selShoptypeAll.action";

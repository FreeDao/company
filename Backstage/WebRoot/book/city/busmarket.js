...
//表格开始
	 datagrid=$('#datagrid').datagrid({
        ...
        iconCls: 'icon-save',
        methord: 'post',
        url: '../Backstage/news/selBusMarket.action',
        pageSize: 20,
        pageList:[5,10,15,20],
        frozenColumns: [[
        ...
        columns: [[
					...
					{ field: 'bmsUserName', title: '昵称', width: 150,align:'center' },
					{ field: 'villageName', title: '小区', width: 150,align:'center' },
...
function newTxl(){
 ...
 function saveTxl(){
	  $('#editForm').form('submit', {
	        url:'../Backstage/news/addBusMarket.action',
	        onSubmit: function () {
...
//批量删除开始
	  if (num >0) { 
		  $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
	            if (data) {
	                $.ajax({
	                    url: '../Backstage/news/delBusMarket.action',
	                    type: 'post',
	                    data: {items: ids.join(',')},	  
	                    success: function (data) {
	                    datagrid.datagrid('reload');

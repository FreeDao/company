...
//表格开始
	 datagrid=$('#datagrid').datagrid({
        ...
        iconCls: 'icon-save',
        methord: 'post',
        url: '../Backstage/news/selLerLottery.action',
        pageSize: 20,
        pageList:[5,10,15,20],
        frozenColumns: [[
        ...
        columns: [[
					...
					{ field: 'comment', title: '中奖内容', width: 150,align:'center'},
					{ field: 'time', title: '有效时间', width: 150,align:'center'}
				]],
        fit:true,
        pagination: true,
        rownumbers: true,
...
//批量删除开始
	  if (num >0) { 
		  $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
	            if (data) {
	                $.ajax({
	                    url: '../Backstage/news/delLerLottery.action',
	                    type: 'post',
	                    data: {items: ids.join(',')},	  
	                    success: function (data) {
	                    datagrid.datagrid('reload');

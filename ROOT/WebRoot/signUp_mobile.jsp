...
<html>
  ...
  <body onload="init()">
    ...
    <form id=myForm action="/query!addSeller" method="post" >
    	...
    	<input type=text id=company name=signUp.company placeholder=请填写公司名称(必填)  />
		</div>
		<div>
    	<!-- <label id=attendeeLabel for=attendee >参会人姓名：</label> -->
    	<input type=text id=attendee name=signUp.attendee placeholder=请填写参会人姓名(必填) />
		</div>
		<div>
    	<!-- <label id=jobLabel for=job >职务：</label> -->
    	<input type=text id=job name=signUp.job placeholder=请填写职务(必填)  />
		</div>
		<div>
    	<!-- <label id=telabel for=tel >联系方式：</label> -->
    	<input type=text id=tel name=signUp.tel placeholder=请填写联系方式(必填)   />
		</div>
		<div>
    	<!-- <label id=inviterLabel for=inviter >邀请人姓名/工号：</label> -->
    	<input type=text id=inviter name=signUp.inviter placeholder=请填写邀请人姓名/工号(必填)  />
		</div>
		<div>
		...
		<div>
			...
			<script type="text/javascript">
				function valid(){
					...
					var inviter = document.getElementById("inviter");
					...
					if(inviter.value == ""){
						alert("邀请人必须填写！");
						inviter.style.border = "1px solid #b94a48";
						/* telLabel.style.color = "#b94a48"; */
						inviter.focus();
						return;
					}

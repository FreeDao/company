...
<li class="Secondmenu2"><a href="#" class="current2">市场</a></li>
...
<div class="fanye">
	<ul>
		<c:choose>
			...
			<c:otherwise>
				<li><a href="${pu.url }?pageNow=1&id=${id}">&laquo;</a></li></c:otherwise>
		</c:choose>
		<c:choose>
			<c:when test="${pu.hasPre }">
				<li><a href="${pu.url }?pageNow=${pu.pageNow - 1}&id=${id}">&lsaquo;</a>
				</li>&nbsp;</c:when>
		...
		<c:forEach items="${pu.navigatePages }" var="pnp">
			<c:choose>
				...
				<c:otherwise>
					<li><a href="${pu.url }?pageNow=${pnp}&id=${id}"> <c:out
		...
		<c:choose>
			<c:when test="${pu.hasNext }">
				<li><a href="${pu.url }?pageNow=${pu.pageNow + 1}&id=${id}">&rsaquo;</a>
				</li>&nbsp;</c:when>
		...
		<c:choose>
			...
			<c:otherwise>
				<li><a href="${pu.url }?pageNow=${pu.pageTotal}&id=${id}">&raquo;</a></li>&nbsp;</c:otherwise>
		</c:choose>
	</ul>
</div>

			...
			<div class="row">
				...
				<div class="span9">
					<div style="margin-top: 5px;">
						<h1 class="page-title">&nbsp; 餐饮美食管理</h1>

						<div class="widget widget-table">

							<div style="margin-bottom: 5px;">
								<a data-toggle="modal" href="bg/product!goAddOrEdit"
								...
								<table class="table table-bordered table-hover">
									...
									<tbody>
										<c:forEach items="${pu.data }" var="pd">
											<tr>
												<td><c:if test="${fn: length(pd[1])>11}">  
									          	${fn: substring(pd[1], 0, 12)}...
									          </c:if> <c:if test="${fn: length(pd[1])<=11}">
									          	${pd[1]}
									          </c:if>
												</td>
												<td><c:if test="${fn: length(pd[2])>11}">  
									          	${fn: substring(pd[2], 0, 12)}...
									          </c:if> <c:if test="${fn: length(pd[2])<=11}">
									          	${pd[2]}
									          </c:if>
												</td>
												<td>${pd[4] }</td>
												<td><img src="${pd[5] }" width="60px" height="40px"
													class="img-polaroid" /></td>
												<td><a data-toggle="modal"
													href="bg/product!goAddOrEdit?product.id=${pd[0] }"
													data-target="#myModal" class="btn"><i
														class="icon-pencil"></i>
														 </a> 
														 <a class="btn"
													onclick="del('bg/product!delProduct?product.id=${pd[0]}')"><i
														class="icon-remove" style="cursor: pointer;"></i> </a>
												<c:if test="${pd[7] == 0 }">
													<a class="btn btn-danger"
													onclick="addTop('bg/product!topToggle?product.id=${pd[0]}')">置顶
														 </a>
												</c:if>
												 <c:if test="${pd[7] != 0 }">
													<a class="btn btn-info"
													onclick="delTop('bg/product!topToggle?product.id=${pd[0]}')">取消置顶
														 </a>
												</c:if>
												</td>
											</tr>

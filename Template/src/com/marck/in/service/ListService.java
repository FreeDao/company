...
public class ListService {
	...
	public void setCommodityList(Commodity commodity, Map<String, Object> map,
			Integer pageNo, Integer pageNum, String sort) {
		//置顶的信息
		String hql = "";
		String temp_id = "";
			...
			if(commodities.size() > 0)
			{
				for(Commodity c : commodities){
					if(!CommonUtil.validParams(c.getSellerId())){
						...
						c.setProvinceId(s.getProvinceId());
		...
		//普通list信息
		String sql = commodityCondition(temp_id,commodity,hql,map,sort);
		
		if(!CommonUtil.validParams(map.get("msg"))){
			return;
		}
		pu = hdb.findSql(sql, Commodity.class, pageNo, pageNum);
		commodities = (List<Commodity>) pu.getData();
		if(commodities.size() > 0)
		{
			for(Commodity c : commodities){
		...
		for( Commodity c : (List<Commodity>)pu.getData()){
			sql = "select (select r.name from region r where r.id="+c.getProvinceId()+") provinceName,(select r.name from region r where r.id="+c.getCityId()+") cityName,(select r.name from region r where r.id="+c.getAreaId()+") areaName,(select t.name from type t where t.id="+c.getTypeId()+")typeName,(select t.name from type t where t.id="+c.getSubTypeId()+") subTypeName from dual";
			Object[] obj = (Object[]) hdb.findUniqueSql(sql);
			c.setProvinceName((String) obj[0]);
			c.setCityName((String)obj[1]);
			c.setAreaName((String)obj[2]);
	...
	private String sellerCondition(String temp_id, Seller seller, String sql, String sort) {
		...
		if(!CommonUtil.validParams(sort)){
			...
			case 3:
				...
				break;
			case 5:
				sql += " order by s.addTime desc,s.id desc ";
				break;
			default:
	 ...
	 * @param map 
	 */
	private String commodityCondition(String temp_id, Commodity commodity, String sql, Map<String, Object> map,String sort) {
		// TODO Auto-generated method stub
		sql = "select c.* from commodity c where  c.menuId="+commodity.getMenuId();
		
		if(!CommonUtil.validParams(temp_id)){
			sql += " and c.id not in ("+temp_id+") ";
		}
		if(!CommonUtil.validParams(commodity.getSupplyDemand())){
			sql +=" and c.supplyDemand="+commodity.getSupplyDemand();
		}
		if(!CommonUtil.validParams(commodity.getCategory())){
			sql += " and c.category="+commodity.getCategory();
		}
		if(!CommonUtil.validParams(commodity.getTypeId())){
			sql += " and c.typeId="+commodity.getTypeId();
		}
		if(!CommonUtil.validParams(commodity.getSubTypeId())){
			sql += " and c.subTypeId="+commodity.getSubTypeId();
		}
		if(!CommonUtil.validParams(commodity.getProvinceId())){
			sql += " and (c.sellerId in (select s.id from seller s where s.provinceId = "+commodity.getProvinceId()+") or c.provinceId="+commodity.getProvinceId()+")";
		}
		if(!CommonUtil.validParams(commodity.getCityId())){
			sql += " and (c.sellerId in (select s.id from seller s where s.cityId = "+commodity.getCityId()+") or c.cityId="+commodity.getCityId()+")";
		}
		if(!CommonUtil.validParams(commodity.getAreaId())){
			sql += " and (c.sellerId in (select s.id from seller s where s.areaId = "+commodity.getAreaId()+") or c.areaId="+commodity.getAreaId()+")";
		}
		if(!CommonUtil.validParams(commodity.getEndTime())){
			sql += " and c.endTime > "+commodity.getEndTime();
		}
		if(!CommonUtil.validParams(commodity.getStatus())){
			sql += " and c.status="+commodity.getStatus();
		}
		...
		if(!CommonUtil.validParams(commodity.getUserId())){
			...
			if( null == seller ){
				...
				sql += " and c.sellerId="+seller.getId();
			}
		}
		...
		if(!CommonUtil.validParams(sort)){
			switch (Integer.parseInt(sort)) {
			case 1:
				sql += " order by c.preference asc ";
				break;
			case 2:
				sql += " order by (select count(*) from comment cc where cc.menuId=c.menuId and cc.pid = c.id ) desc ";
				break;
			case 3:
				sql += " order by (select sum(cc.level) from comment cc where cc.menuId=c.menuId and cc.pid = c.id ) desc ";
				break;
			case 5:
				sql += " order by c.addTime desc,c.id desc ";
				break;
			default:
				sql += " order by ACOS(SIN(("+commodity.getLat()+" * 3.1415) / 180 ) *SIN((c.lat * 3.1415) / 180 ) + COS(("+commodity.getLat()+" * 3.1415) / 180 ) * COS(( c.lat * 3.1415) / 180 ) *COS(("+commodity.getLng()+" * 3.1415) / 180 - (c.lng * 3.1415) / 180 ) ) * 6380 asc,c.addTime DESC  ";
				break;
			}
		}

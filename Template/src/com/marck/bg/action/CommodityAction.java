...
public class CommodityAction extends BaseAction {
		...
		try {
			...
			if(systemAgrs.getModel() == 3){
				...
				if(!CommonUtil.validParams(typeTemp,typeNameTemp)){
					...
					for(int i = 0 ; i < temp.length ; i++){
						...
						List<Type> subTypes = commodityService.selAllCommodityType(temp[i]);
						map.put("name" , nameTemp[i] );
						map.put("data", subTypes);
						types.add(map);
					}
				}
			}
			...
			if( CommonUtil.validParams(commodity) ){
				...
				if(systemAgrs.getModel() == 3){
					...
					if(!CommonUtil.validParams(typeTemp)){
						String[] temp = typeTemp.split(",");
						
						for(int i = 0 ; i < temp.length ; i++){
							List<Type> pTypes = (List<Type>) types.get(i).get("data");
							List<Type> subTypes = commodityService.selCommodityType(commodity,temp[i]);
	...
	public String addOrUpdateCommodity(){
		try {
			...
			if(commodityService.addOrUpdateCommodity(commodity,menuId,img,imgContentType,imgFileName,filepath,projectUrl,uploadFolder,imgs,types)){
				commodity = null;
	...
	public String delCommodity(){
		try {
			String msg = commodityService.delCommodity(commodity);
			if( null != msg ){

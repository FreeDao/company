...
public class CommodityService {
	...
	public boolean addOrUpdateCommodity(Commodity commodity, Integer menuId, File img,
			...
			String projectUrl, String uploadFolder, List<Img> imgs, String[] types) throws IOException {
		// TODO Auto-generated method stub
		boolean isNew = false;
		...
		if( null != types){
			for( int i = 0 ; i < types.length ;i++ ){
				...
				ct.setTypeId(Integer.parseInt(types[i]));
				hdb.saveOrUpdate(ct);
			}
		}
	...
	public String delCommodity(Commodity commodity) {
		...
		String hql = "from Shop s where s.commodityId ="+commodity.getId();
		List<?> list = hdb.findHql(hql);
		if(list.size() > 0){
			return "用户已生成订单，无法删除。";
		}
		
		hql = "from Comment c where c.menuId = "+commodity.getMenuId()+" and c.pid="+commodity.getId();
	 ...
	 * @return
	 */
	public List<Type> selCommodityType(Commodity commodity, String temp) {
		// TODO Auto-generated method stub
		return (List<Type>) hdb.findHql("select t from CommodityType ct,Type t where ct.typeId = t.id and t.type = "+Integer.parseInt(temp)+"  and ct.commodityId ="+commodity.getId());
	}
	 ...
	 * @return
	 */
	public List<Type> selAllCommodityType(String temp) {
		// TODO Auto-generated method stub
		return (List<Type>) hdb.findHql("from Type t where t.type = "+Integer.parseInt(temp));
	}
}

...
public class SellerAction extends BaseAction{
	...
	public String addOrUpdateSeller(){
		try {
			...
			if(null == seller  || CommonUtil.validParams(seller.getUserId(),seller.getName())){
				map.put("msg", "缺少必传参数");
				map.put("code", 0);
				return SUCCESS;
			}
	...
	public String addOrUpdateSupplyDemand(){
		try {
			...
			if(null == commodity  || CommonUtil.validParams(commodity.getUserId(),commodity.getTitle(),commodity.getEndTime(),commodity.getStatus())){
				map.put("msg", "缺少必传参数");
				map.put("code", 0);
				return SUCCESS;
			}

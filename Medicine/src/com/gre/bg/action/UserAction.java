...
public class UserAction extends BaseAction{
	...
	public String selAllUser(){
		try {
			...
			pu.setUrl("bg/user!selAllUser");
			return "userList";
		} catch (Exception e) {
			// TODO: handle exception
			log.error("方法selAllUser出错");
		}
		return ERROR;
	}
	*/
	...
	public String delAccount(){
		try {
			List<UserCollect> ucs = (List<UserCollect>) userService.findCollectListByUserId(UserCollect.class,user.getId());
			for(UserCollect uc : ucs){
				userService.delObj(uc);
			}
			userService.delObjById(User.class,user.getId());
			request.setAttribute("msg", 1);
			return selAllUser();
		} catch (Exception e) {
			// TODO: handle exception
			request.setAttribute("msg", 0);
			log.debug("UserAction中方法delAccount出错");
			return selAllUser();
		}
	}
	*/

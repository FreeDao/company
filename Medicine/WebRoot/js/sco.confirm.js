...
;(function($, undefined) {
	...
	function Confirm(options) {
		...
		var $modal = $(this.options.target,top.document);
		if (!$modal.length) {
			$modal = $('<div class="modal" id="' + this.options.target.substr(1) + '"><div class="modal-body inner"/><div class="modal-footer"><a class="btn cancel" href="#" data-dismiss="modal">取消</a> <a href="#" class="btn btn-danger" data-action="1" target="mainFrame">确定</a></div></div>').appendTo(this.options.appendTo).hide();
	...
	$.fn[pluginName].defaults = {
		...
		,appendTo: top.document.body//'body'//$("body",top.document)	// where should the modal be appended to (default to document.body). Added for unit tests, not really needed in real life.
	};

	$(document).on('click.' + pluginName, '[data-trigger="confirm"]', function() {
		$(this)[pluginName]();
		return false;
	}).on('click.'+pluginName,'[data-action="1"]',function(e){
		e.preventDefault();
		mainFrame.location.href=$(this).attr("href");

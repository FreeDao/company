...
<h1 class="page-title">&nbsp; 
				...
				<div class="widget widget-table">
					...
					<div style="margin-bottom: 5px;">
						<a data-trigger="modal" href="bg/exhibition!goAddOrEdit" data-title="添加展览展会资讯" class="btn btn-primary"><i class="icon-plus" ></i>添加</a>
					</div>
								...
								<table class="table table-bordered table-hover">
									...
									<c:forEach items="${pu.data }" var="pd">
										<tr>
											...
											<td>
											  <c:if test="${fn: length(pd.addr)>11}">  
									          	${fn: substring(pd.addr, 0, 12)}...
									          </c:if> 
									          <c:if test="${fn: length(pd.addr)<=11}">
									          	${pd.addr}
									          </c:if> 
											...
											<td>${pd.endTime }</td>
											<td>
											<a class="btn" data-trigger="modal" href="bg/exhibition!goAddOrEdit?exhibition.id=${pd.id }" data-title="修改展览展会资讯">
											<i class="icon-pencil"></i>
											</a>
											<a data-trigger="confirm" href="bg/exhibition!delExhibition?exhibition.id=${pd.id}" class="btn">

...
<h1 class="page-title">&nbsp; 
				...
				<div class="widget widget-table">
					...
					<div style="margin-bottom: 5px;">
						<a data-trigger="modal" href="bg/invitation!goAddOrEdit" data-title="添加招标信息" class="btn btn-primary"><i class="icon-plus" ></i>添加</a>
					</div>
								...
								<table class="table table-bordered table-hover">
									...
									<c:forEach items="${pu.data }" var="pd">
										<tr>
											...
											<td>
											<c:choose>
												<c:when test="${pd.type == 0 }">招标采购</c:when>
												<c:when test="${pd.type == 1 }">更正通告</c:when>
												<c:when test="${pd.type == 2 }">中标结果</c:when>
												<c:otherwise>无</c:otherwise>
											</c:choose>
											</td>
											<td>
											<a class="btn" data-trigger="modal" href="bg/invitation!goAddOrEdit?invitation.id=${pd.id }" data-title="修改招标信息">
											<i class="icon-pencil"></i>
											</a>
											<a data-trigger="confirm" href="bg/invitation!delInvitation?invitation.id=${pd.id}" class="btn">
					...
					<c:forEach items="${pu.navigatePages }" var="pnp">
						<c:choose>
							...
							<c:otherwise>
								<li>sh<a href="${pu.url }?pageNow=${pnp}">
